...
def execute_spare_mutation(question, confirmed=False):
    ...
    if not matches:
        ...
        for r in _v14_load_spares():
            sheet_name = str(r.get("sheet") or "").upper().replace("-", "").strip()
            instrument = str(r.get("instrument") or "").upper().strip()
            item = str(r.get("item") or "").upper().strip()

            # Known spare families are matched ONLY by worksheet.
            # This prevents SOV from accidentally matching tags/items
            # such as 1SV, 6SV and 9SV from other worksheets.
            known_families = {
                "SOV", "FSV", "PCV", "FCV", "MCV",
                "PT", "LT", "FT", "RTD"
            }

            if ident_norm in known_families:
                if ident_norm == sheet_name:
                    category_matches.append(r)
                continue

            # Non-family category identifiers may use clear
            # instrument/item prefixes.
            if (
                instrument.startswith(ident_norm + "_") or
