...
def _anvi_conversational_maintenance_answer(
    ...
    # ------------------------------------------------------------
    # DIRECT PLANT MEMORY BRIDGE
    # ------------------------------------------------------------
    # Do not depend exclusively on _ANVI_CONVERSATION_CONTEXT.
    # Troubleshooting can arrive directly with a resolved equipment tag.
    #
    # This guarantees PT_303 and PT-303 reach the same verified memory.
    # ------------------------------------------------------------
    ...
    try:
        ...
        if bridge_tag:
            direct_memory_records = _anvi_troubleshooting_memory_bridge(
                question,
                tag=bridge_tag,
    ...
    if direct_memory_records:
        existing = data.get("plant_memory_records")
