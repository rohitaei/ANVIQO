...
def _html_page(path: Path):
    if not path.is_file(): return Response(f"ANVIQO page unavailable: {path.name}", status=500, mimetype="text/plain")
...
def _admin() -> bool:
    actor = _actor(); return bool(actor["user_id"] and actor["organization_id"] and actor["role"] in {"OWNER", "ADMIN"})

def _owner() -> bool: return _actor()["role"] == "OWNER"
