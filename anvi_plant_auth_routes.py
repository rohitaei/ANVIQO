...
def _plant_belongs_to_actor_org(plant_id: str) -> bool:
    actor = _actor()
    if not plant_id or not actor["organization_id"]:
        return False
...
def _user_in_actor_org(username: str) -> bool:
    ...
    with store._connect() as conn:
        cur = conn.cursor()
        cur.execute(
            f"""SELECT 1 FROM anviqo_users u
                JOIN anviqo_memberships m ON m.user_id=u.user_id
                WHERE u.external_username={p} AND m.organization_id={p} AND m.status='ACTIVE'
                LIMIT 1""",
            (username, actor["organization_id"]),
        )
        return cur.fetchone() is not None
...
def admin_plants():
    ...
    with store._connect() as conn:
        cur = conn.cursor()
        cur.execute(
            f"SELECT plant_id,name,slug,status FROM anviqo_plants WHERE organization_id={p} ORDER BY name",
...
def create_plant_user():
    ...
    if len(password) < 8:
        return jsonify({"status": "INVALID", "message": "Password must contain at least 8 characters"}), 400
    if role not in store.ROLE_PERMISSIONS:
        return jsonify({"status": "INVALID", "message": "Unsupported role"}), 400
    # Only OWNER may create elevated ADMIN/OWNER accounts.
    if role in {"OWNER", "ADMIN"} and not _owner():
        return jsonify({"status": "FORBIDDEN", "message": "Only OWNER may provision ADMIN/OWNER accounts"}), 403
    if not _plant_belongs_to_actor_org(plant_id):
        return jsonify({"status": "FORBIDDEN", "message": "Plant is not active in your organization"}), 403
    if _user_in_actor_org(username):
        return jsonify({"status": "CONFLICT", "message": "User already has an active membership in this organization"}), 409
    try:
        ...
        try:
            store.record_audit(_actor(), "CREATE_USER", "USER", user_id, {"plant_id": plant_id, "role": role})
        except Exception:
            pass
...
def change_plant_user_password():
    ...
    if len(password) < 8:
        return jsonify({"status": "INVALID", "message": "Password must contain at least 8 characters"}), 400
    if not _user_in_actor_org(username):
        return jsonify({"status": "FORBIDDEN", "message": "User is not active in your organization"}), 403
    try:
        plant_auth.set_password(username, password)
        try:
            store.record_audit(_actor(), "CHANGE_PASSWORD", "USER", username, {})
        except Exception:
            pass
...
__all__ = ["plant_login_interceptor", "session_context", "admin_plants", "create_plant_user", "change_plant_user_password", "my_plants"]
