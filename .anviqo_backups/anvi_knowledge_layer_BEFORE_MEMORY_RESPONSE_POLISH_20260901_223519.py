...
def _pci_memory_route(question):
    ...
    return {
        "answer": (
            f"I found {len(verified)} verified previous human "
            "field experience record(s)"
            + (f" associated with {tag}." if tag else ".")
            + " These records are evidence of previous experience "
              "and are not treated as automatic causation."
        ),
        "domain": "plant_memory",
        "evidence": "verified plant memory",
        "count": len(verified),
