...
def _v16_identifier(q):
    ...
    # Example: PCI_BOKE 4M310-08
    m = _v16_re.search(
        r"\\b(PCI_[A-Z0-9]+(?:[ _-]+[A-Z0-9]+)*-[A-Z0-9]+)\\b",
        q,
        _v16_re.I
    )
    if m:
    ...
    # More permissive PCI_ identifier, stopping before "spare(s)".
    m = _v16_re.search(
        r"\\b(PCI_[A-Z0-9][A-Z0-9 _-]*?)(?=\\s+spares?\\b|\\s*$)",
        q,
        _v16_re.I
    )
    if m:
    ...
    # Family-only identifiers are intentionally retained for
    # compatibility, but the executor must reject ambiguous
    # multi-record families before mutation.
    for family in (
        "SOV", "FSV", "PCV", "FCV", "MCV",
        "PT", "LT", "FT", "RTD"
    ):
