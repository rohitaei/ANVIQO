...
def ask_anvi(question):
    ...
    try:
        ...
        if troubleshooting:
            ...
            # ------------------------------------------------------------
            # VERIFIED PCI CONTEXT TROUBLESHOOTING
            # ------------------------------------------------------------
            # Use only verified PCI topology. Do not claim a failure cause.
            # Existing V5 maintenance intelligence remains authoritative.
            if isinstance(previous, dict):
                tag = str(
                    previous.get("tag")
                    or previous.get("instrument_tag")
                ...
                if tag:
                    ...
                    tb = str(
                        previous.get("tb_name")
                        or previous.get("tb")
                        or previous.get("terminal_block")
                        or ""
                    ).strip()

                    terminals = str(
                        previous.get("tb_no")
                        or previous.get("terminal")
                        or previous.get("terminals")
                        or previous.get("terminal_reference")
                        or ""
                    ).strip()

                    io_type = str(
                        previous.get("io_type")
                        or ""
                    ).strip()

                    role = str(
                        previous.get("process_role")
                        or ""
                    ).strip()
                    ...
                    if description:
                        checks.append(
                            f"1. Confirm the field instrument: {tag} ({description})."
                        )
                    else:
                        checks.append(
                            f"1. Confirm the field instrument {tag}."
                        )

                    if role:
                        checks.append(
                            f"2. Confirm the process condition for {role}."
                        )
                    else:
                        checks.append(
                            "2. Confirm the actual process condition at the transmitter."
                        )

                    checks.append(
                        "3. Verify instrument power and transmitter health at the field."
                    )

                    if io_type:
                        checks.append(
                            f"4. Verify the {io_type} signal from the transmitter."
                        )
                    else:
                        checks.append(
                            "4. Verify the transmitter output signal and loop current."
                        )
                    ...
                    if tb or terminals:
                        ...
                        if terminals:
                            tb_text += (
                                ...
                                if tb_text
                                else f"terminals {terminals}"
                            )
                        checks.append(
                            f"5. Check the verified field termination at {tb_text}."
                        )

                    if panel:
                        checks.append(
                            f"6. Check the signal path through panel {panel}."
                        )

                    if plc:
                        checks.append(
                            f"7. Check the corresponding PLC input at {plc}."
                        )

                    checks.append(
                        "8. Compare the field measurement, loop signal and PLC/SCADA indication before concluding that the transmitter has failed."
                    )

                    answer = (
                        f"ANVI troubleshooting guidance for {tag}:\n"
                        f"Verified location: {area or 'not specified'}.\n"
                        + "\n".join(checks)
                        + "\n\n"
                        "ANVI cannot establish a specific failure cause from the currently verified maintenance evidence. "
                        "These are diagnostic checks based on the verified PCI signal path only."
                    )
                    ...
                    return {
                        ...
                        "human_decision_required": True,
                        "context_tag": tag,
                        "evidence": "verified PCI topology + existing V5 maintenance intelligence",
                        "plc_write": False,
                        "scada_control": False,
                    }

            # Existing V5 maintenance path remains the fallback.
            if previous:
