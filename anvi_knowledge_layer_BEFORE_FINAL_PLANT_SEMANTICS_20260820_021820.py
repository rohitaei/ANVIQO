...
def _plant_evidence_answer(q):
    """
    Produce a plant-wide evidence response from the unified context.

    This is not a prediction engine. It only interprets existing
    evidence and preserves the existing V5 safety boundary.
    """
    import json
    ...
    for area in areas:
        ...
        critical = (
            area.get("critical")
            or area.get("critical_equipment")
            or 0
        )
    ...
    # Highest-risk areas first where a score exists.
    attention.sort(
        key=lambda x: (
    ...
    response = {
        ...
        "plant_health_score": evidence.get("plant_health_score"),
        "healthy": evidence.get("healthy"),
        "warning": evidence.get("warning"),
        "critical": evidence.get("critical"),
        "changed": evidence.get("changed"),
        "active_events": evidence.get("active_events"),
        "area_count": evidence.get("area_count"),
        "areas_requiring_attention": attention,
        "evidence_sources": evidence.get("source", []),
        "explanation": (
            "ANVI is using the existing plant evidence context to "
            "identify current conditions and risk indicators. "
            "These indicators do not establish that a specific "
            "failure will occur."
        ),
