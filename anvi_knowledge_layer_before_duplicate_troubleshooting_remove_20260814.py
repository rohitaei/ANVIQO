...
def ask_anvi(question):
    ...
    try:
        ...
        if troubleshooting:
            ...
            # --------------------------------------------------------
            # VERIFIED PCI CONTEXT -> TECHNICIAN TROUBLESHOOTING
            # --------------------------------------------------------
            if isinstance(previous, dict):
                tag = str(
                    previous.get("tag")
                ...
                if tag:
                    ...
                    panel = str(previous.get("panel") or "").strip()
                    ...
                    checks = [
                        ...
                        "2. Verify the actual process condition at the transmitter."
                    ]

                    if role:
                        checks[1] = (
                            f"2. Verify the actual process condition for {role}."
                        )
                    ...
                    if io_type:
                        checks.append(
                            f"4. Verify the {io_type} output signal from the transmitter."
                        )
                    else:
                        checks.append(
                    ...
                    if tb or terminals:
                        ...
                        checks.append(
                            f"5. Check the verified field termination at {termination}."
                        )
                    ...
                    answer = (
                        ...
                        f"Verified area: {area or 'not specified'}.\n\n"
                        + "\n".join(checks)
                        + "\n\n"
                        "ANVI cannot establish a specific failure cause from the currently verified maintenance evidence. "
                        "These checks are based on the verified PCI signal path. "
                        "Human verification, permit requirements, isolation and risk assessment remain mandatory before field intervention."
                    )
                    ...
                    return {
                        ...
                        "equipment": tag,
                        "context_tag": tag,
                        "evidence": "verified PCI topology",
                        "read_only": True,
                        "human_decision_required": True,
                        "plc_write": False,
                        "scada_control": False,
            ...
            # --------------------------------------------------------
            # EXISTING V5 MAINTENANCE FALLBACK
            # --------------------------------------------------------
