...
def parse_shift_report_text(
    ...
    *,
    ...
    spreadsheet. Blank cells are ignored. The report's hourly period is mapped
    to the start of that hour. No units or quality values are invented.
    """
    ...
    for row in rows[header_index + 1 :]:
        ...
        if hour > 23:
            raise ValueError(f"invalid hourly period: {period}")

        timestamp = report_date.replace(hour=hour, minute=0, second=0, microsecond=0).isoformat()
...
__all__ = ["parse_shift_report_text"]
