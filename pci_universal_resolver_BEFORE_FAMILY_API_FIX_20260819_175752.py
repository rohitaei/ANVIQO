...
def family_records(query, records=None):
    """
    Return every verified PCI record belonging to the resolved
    equipment/tag family.

    This is intentionally based only on the authoritative PCI
    database. No tag is invented.
    """
    resolved, match_type = resolve(query, records)
...
def all_records_for_tag(query, records=None):
    """
    Explicit alias for complete verified PCI coverage.
    """
...
def io_summary(query, records=None):
    """
    Return the complete I/O breakdown for a resolved tag/equipment family.
    """
