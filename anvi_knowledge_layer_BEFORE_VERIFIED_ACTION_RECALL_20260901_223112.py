...
def _pci_memory_route(question):
    """
    ANVIQO Plant Memory V1.1 conversational adapter.

    Uses the verified Plant Memory V1 backend.

    Rules:
      - Retrieval is evidence-backed.
      - Only verified=True records are returned.
      - No automatic causation is claimed.
      - No unverified conversational report is automatically stored.
    ...
    try:
        import plant_memory as pm
    except Exception as exc:
        return {
            "answer": (
                "ANVIQO Plant Memory is installed but could not be "
                f"loaded: {exc}"
            ),
            "domain": "plant_memory",
            "evidence": "none",
    ...
    # --------------------------------------------------------
    # Resolve an explicit verified PCI tag when present.
    # --------------------------------------------------------
    tag = None
    ...
    # --------------------------------------------------------
    # Also use current conversational PCI context.
    # --------------------------------------------------------
    if not tag:
        try:
            context = globals().get(
    ...
    # --------------------------------------------------------
    # VERIFIED MEMORY SEARCH
    # --------------------------------------------------------
    try:
        results = pm.search_memory(
            query=q,
            tag=tag
        )
    except TypeError:
        # Compatibility with implementations that accept only
        # positional query/tag arguments.
        try:
            results = pm.search_memory(q, tag=tag)
        except Exception:
            results = []
    except Exception:
        results = []
    ...
    # --------------------------------------------------------
    # Explicit "what worked before" / action recall.
    # Prefer records containing a verified maintenance action.
    # --------------------------------------------------------
    action_question = any(x in ql for x in [
        "what worked before",
        "what action fixed",
        "which action fixed",
        "what fixed it",
    ...
    # Defensive verified-only filter.
    results = [
        r for r in results
    ...
    if not results:
        return {
            "answer": (
                "I could not find verified Plant Memory evidence "
                + (f"for {tag}. " if tag else "matching this request. ")
                + "I will not invent plant history or recommend an "
                  "action without supporting evidence."
            ),
            "domain": "plant_memory",
    ...
    if action_question:
        lines.append(
            f"I found {len(results)} verified previous maintenance "
            "experience record(s)."
        )
    else:
        lines.append(
            f"I found {len(results)} verified previous plant "
            "memory record(s)."
        )

    if tag:
        lines.append(f"Equipment/tag: {tag}.")

    lines.append(
        "These records are historical evidence; ANVI does not "
        "treat them as automatic causation or automatic authorization."
    )
    ...
    for r in results[:10]:
        item = {
            ...
            "maintenance_action": r.get(
                "maintenance_action"
            ),
            "finding": r.get("finding"),
            "confirmation_evidence": r.get(
                "confirmation_evidence"
            ),
            "outcome": r.get("outcome"),
            "recovery_status": r.get(
                "recovery_status"
            ),
    ...
    # Human-readable answer for the most relevant record.
    first = reports[0]

    if action_question:
        action = first.get("maintenance_action") or "Not recorded."
        finding = first.get("finding") or "Not recorded."
        outcome = first.get("outcome") or "Not recorded."
        ...
        lines.extend([
            "",
            f"Memory: {first.get('memory_id')}",
            f"Event: {first.get('event') or 'Not recorded.'}",
            f"Finding: {first.get('finding') or 'Not recorded.'}",
            f"Action: {first.get('maintenance_action') or 'Not recorded.'}",
            f"Outcome: {first.get('outcome') or 'Not recorded.'}",
        ])
