...
Stores plant onboarding metadata and source documents. The active user path is
now the clean direct plant-data importer; no ingestion job, queue, worker or
polling is required. Frozen V5 intelligence is not modified.
"""
...
def _placeholder(): return store._placeholder()
def _actor():
...
def onboarding_status(plant_id):
 ...
 with store._connect() as conn:
  cur=conn.cursor();cur.execute(f"SELECT p.name,p.slug,o.industry,o.location,o.description,o.status,o.updated_at FROM anviqo_plants p LEFT JOIN anviqo_plant_onboarding o ON o.plant_id=p.plant_id WHERE p.plant_id={p}",(plant_id,));row=cur.fetchone()
...
def update_onboarding_profile(plant_id):
 ...
 with store._connect() as conn:conn.cursor().execute(f"UPDATE anviqo_plant_onboarding SET industry={p},location={p},description={p},status='DATA_UPLOADED',updated_at={p} WHERE plant_id={p} AND organization_id={p}",(industry,location,description,_now(),plant_id,a["organization_id"]))
...
def upload_onboarding_document(plant_id):
 ...
 for f in uploaded:
  filename=(f.filename or "").strip().replace("\\","/").split("/")[-1];ext=filename.rsplit(".",1)[-1].lower() if "." in filename else ""
 ...
 with store._connect() as conn:conn.cursor().execute(f"UPDATE anviqo_plant_onboarding SET status='DATA_UPLOADED',updated_at={p} WHERE plant_id={p} AND organization_id={p}",(_now(),plant_id,a["organization_id"]))
