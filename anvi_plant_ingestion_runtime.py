...
def _records(filename,raw):
    ...
    if ext=='json':
        ...
        return out
    if ext in {'xlsx','xls'}:
        return [{'external_id':hashlib.sha256((filename+s+'|'.join(v)).encode()).hexdigest()[:20],'name':' | '.join(v),'area':s,'record_type':'TABULAR','source':filename} for s,v in _xlsx(raw)]
...
def register(app):
    init_schema(); from flask import jsonify,request
    ...
    def plant_knowledge_route():
        ...
        with store._connect() as conn:
            ...
            if q:
                sql+=f" AND (LOWER(name) LIKE {_p()} OR LOWER(area) LIKE {_p()} OR LOWER(service) LIKE {_p()} OR LOWER(tag) LIKE {_p()} OR LOWER(content) LIKE {_p()})"; like=f'%{q}%'; args += [like]*5
