...
_JOB_LOCK=threading.Lock(); _JOBS={}
...
def _xlsx_records(raw):
 ...
 try:
  for ws in wb.worksheets:
   ...
   headers=[]
   for i,v in enumerate(first):headers.append(str(v).strip() if v is not None and str(v).strip() else f"column_{i+1}")
...
def _xlsx(raw): return list(_xlsx_records(raw))
def _records(filename,raw):
 ext=filename.lower().rsplit('.',1)[-1] if '.' in filename else ''
 if ext=='xlsx':return _xlsx(raw)
 if ext=='csv':
  out=[]
  ...
  return out
 if ext=='json':
  data=json.loads(raw.decode('utf-8',errors='replace'));data=data.get('records') or data.get('data') or [data] if isinstance(data,dict) else data;out=[]
...
def _record_to_row(plant_id,document_id,digest,rec,org):
 ...
 return (kid,org,plant_id,document_id,rec['record_type'],rec['external_id'],rec.get('name',''),rec.get('area',''),rec.get('service',''),rec.get('asset_type',''),rec.get('tag',''),rec.get('parent_id',''),rec.get('source',''),json.dumps(meta),content)
...
def ingest_plant(plant_id,actor=None):
 ...
 total=documents=0;errors=[];batch=[];BATCH=250
 for document_id,filename,raw,digest in docs:
  try:
   records=_xlsx_records(bytes(raw)) if filename.lower().endswith('.xlsx') else _records(filename,bytes(raw))
   identity={'organization_id':a['organization_id'],'plant_id':plant_id,'name':plant[0],'industry':plant[1] or ''}
   for rec in records:
    package=build_onboarding_package(identity,[rec])
    for normalized in package.get('records',[]):
     batch.append(_record_to_row(plant_id,document_id,digest,normalized,a['organization_id']));total+=1
     if len(batch)>=BATCH:_insert_rows(batch);batch=[]
   documents+=1
  except Exception as exc:errors.append({'filename':filename,'error':str(exc)})
 if batch:_insert_rows(batch)
 status='READY' if documents and not errors else 'BLOCKED' if errors and not total else 'DATA_UPLOADED'
 with store._connect() as conn:
  cur=conn.cursor();cur.execute(f"UPDATE anviqo_plant_onboarding SET status={p},updated_at={p} WHERE plant_id={p} AND organization_id={p}",(status,_now(),plant_id,a['organization_id']))
...
def _run_job(plant_id,actor):
 try:result=ingest_plant(plant_id,actor);result['job_status']='COMPLETED'
 except Exception as exc:result={'status':'ERROR','plant_id':plant_id,'message':str(exc),'job_status':'FAILED','safety':dict(SAFETY)}
 with _JOB_LOCK:_JOBS[plant_id]=result
 try:
  p=_p();status='READY' if result.get('status')=='OK' and not result.get('errors') else 'BLOCKED'
  with store._connect() as conn:conn.cursor().execute(f"UPDATE anviqo_plant_onboarding SET status={p},updated_at={p} WHERE plant_id={p}",(status,_now(),plant_id))
 except Exception:pass
def register(app):
 ...
 def ingest_route(plant_id):
  ...
  p=_p()
  try:
   with store._connect() as conn:conn.cursor().execute(f"UPDATE anviqo_plant_onboarding SET status='INDEXING',updated_at={p} WHERE plant_id={p}",(_now(),plant_id))
  except Exception:pass
