"""ANVIQO tenant-scoped universal plant ingestion runtime.

Consumes approved onboarding documents and converts them into the existing
ANVIQO-ONBOARDING-V1 contract. This is an adapter/indexing layer only: it does
not modify frozen V5 intelligence and never controls PLC/SCADA.
"""
...
def _plant_ok(plant_id):
    a = _actor()
    if not _admin() or not plant_id:
        return False
    with store._connect() as conn:
        cur = conn.cursor()
        cur.execute(f"SELECT 1 FROM anviqo_plants WHERE plant_id={_p()} AND organization_id={_p()} AND status='ACTIVE' LIMIT 1", (plant_id, a["organization_id"]))
...
def _records(filename, raw):
    ...
    if ext in {"csv"}:
        ...
        return [{**row, "source": filename, "record_type": "DATA"} for row in csv.DictReader(io.StringIO(text))]
    if ext == "json":
        data = json.loads(raw.decode("utf-8", errors="replace"))
        if isinstance(data, dict):
            data = data.get("records") or data.get("data") or [data]
        return [{**r, "source": filename} for r in data if isinstance(r, dict)]
    if ext in {"xlsx", "xls"}:
        out = []
        for sheet, vals in _xlsx_rows(raw):
            out.append({"external_id": hashlib.sha256((filename+sheet+"|"+"|".join(vals)).encode()).hexdigest()[:20], "name": " | ".join(vals), "area": sheet, "record_type": "TABULAR", "source": filename})
        return out
    if ext == "docx":
        text = _decode_docx(raw)
    elif ext == "pdf":
    ...
    return [{"external_id": hashlib.sha256((filename+str(i)).encode()).hexdigest()[:20], "name": filename, "record_type": "DOCUMENT", "source": filename, "content": chunk} for i, chunk in enumerate(chunks)]


def ingest_plant(plant_id):
    if not _plant_ok(plant_id):
        raise PermissionError("OWNER/ADMIN access to this plant is required")
    init_schema()
    a = _actor(); p = _p()
    with store._connect() as conn:
        cur = conn.cursor()
        cur.execute(f"SELECT p.name,o.industry FROM anviqo_plants p LEFT JOIN anviqo_plant_onboarding o ON o.plant_id=p.plant_id WHERE p.plant_id={p} AND p.organization_id={p}", (plant_id, a["organization_id"]))
        plant_row = cur.fetchone()
        cur.execute(f"SELECT document_id,filename,content,sha256 FROM anviqo_plant_documents WHERE plant_id={p} AND organization_id={p} ORDER BY created_at", (plant_id, a["organization_id"]))
        docs = cur.fetchall()
    if not plant_row:
        raise ValueError("Plant not found")
    plant_name, industry = plant_row
    total = 0; documents = 0; errors = []
    with store._connect() as conn:
        cur = conn.cursor()
        for doc in docs:
            document_id, filename, raw, digest = doc
            try:
                records = _records(filename, bytes(raw))
                identity = {"organization_id": a["organization_id"], "plant_id": plant_id, "name": plant_name, "industry": industry or ""}
                package = build_onboarding_package(identity, records) if records else {"records": []}
                for rec in package.get("records", []):
                    ...
                    meta.update({"ingestion_version": INGESTION_VERSION, "document_sha256": digest})
                    values = (kid,a["organization_id"],plant_id,document_id,rec["record_type"],rec["external_id"],rec.get("name", ""),rec.get("area", ""),rec.get("service", ""),rec.get("asset_type", ""),rec.get("tag", ""),rec.get("parent_id", ""),rec.get("source", filename),json.dumps(meta),rec.get("metadata", {}).get("content", "") if isinstance(rec.get("metadata"), dict) else "",_now())
                    if store._is_sqlite():
                        cur.execute(f"INSERT OR REPLACE INTO anviqo_plant_knowledge(knowledge_id,organization_id,plant_id,document_id,record_type,external_id,name,area,service,asset_type,tag,parent_id,source,metadata,content,created_at) VALUES({','.join([p]*16)})", values)
                    else:
                        cur.execute(f"INSERT INTO anviqo_plant_knowledge(knowledge_id,organization_id,plant_id,document_id,record_type,external_id,name,area,service,asset_type,tag,parent_id,source,metadata,content) VALUES({','.join([p]*15)}) ON CONFLICT(plant_id,external_id,source) DO UPDATE SET name=EXCLUDED.name,area=EXCLUDED.area,service=EXCLUDED.service,asset_type=EXCLUDED.asset_type,tag=EXCLUDED.tag,metadata=EXCLUDED.metadata,content=EXCLUDED.content", values[:-1])
                    total += 1
                documents += 1
            except Exception as exc:
                errors.append({"filename": filename, "error": str(exc)})
        status = "READY" if documents and not errors else ("BLOCKED" if errors and not total else "DATA_UPLOADED")
        cur.execute(f"UPDATE anviqo_plant_onboarding SET status={p},updated_at={p} WHERE plant_id={p}", (status,_now(),plant_id))
...
def register(app):
    ...
    def ingest_route(plant_id):
        ...
        try:
            return jsonify(ingest_plant(plant_id))
        except Exception as exc:
            return jsonify({"status":"ERROR","message":str(exc),"safety":dict(SAFETY)}),400
    @app.get("/api/plant/knowledge")
    def plant_knowledge_route():
        a = _actor(); plant_id = a.get("plant_id")
        if not a.get("user_id") or not plant_id:
            return jsonify({"status":"UNAUTHORIZED"}),401
        q = str(request.args.get("q", "")).strip().lower()
        limit = min(max(int(request.args.get("limit", "50")),1),100)
        with store._connect() as conn:
            cur = conn.cursor()
            sql = f"SELECT record_type,external_id,name,area,service,asset_type,tag,parent_id,source,metadata,content FROM anviqo_plant_knowledge WHERE plant_id={_p()}"
            args=[plant_id]
            if q:
                sql += f" AND (LOWER(name) LIKE {_p()} OR LOWER(area) LIKE {_p()} OR LOWER(service) LIKE {_p()} OR LOWER(tag) LIKE {_p()} OR LOWER(content) LIKE {_p()})"
