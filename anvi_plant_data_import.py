...
VERSION = "ANVIQO-DIRECT-PLANT-DATA-V1.2"
...
def _text(raw, ext):
    if ext in {"txt","log","csv"}: return raw.decode("utf-8-sig",errors="replace")
    if ext == "docx":
...
def _records(filename,raw):
    ...
    if ext=="csv":
        ...
        return
    if ext=="json":
        data=json.loads(raw.decode("utf-8",errors="replace")); data=data.get("records") or data.get("data") or [data] if isinstance(data,dict) else data
...
def _insert_batch(plant_id,org_id,document_id,digest,records):
    p=_p(); rows=[]
    for raw in records:
        item=normalize_record(raw,str(raw.get("record_type") or "ASSET"))
        if hasattr(item,"__dict__"): item=item.__dict__
...
def import_plant_data(plant_id,actor):
    ...
    for document_id,filename,digest in docs:
        try:
            # Fetch one document at a time so multiple large workbooks are never resident together.
            with store._connect() as conn:
            ...
            raw=bytes(row[0]); records=_records(filename,raw); batch=[]; batch_size=50
            for rec in records:
                batch.append(rec)
                if len(batch)>=batch_size: total+=_insert_batch(plant_id,actor["organization_id"],document_id,digest,batch); batch=[]
...
def register(app):
    ...
    def import_route(plant_id):
        ...
        try:return jsonify(import_plant_data(plant_id,actor)),200
    ...
    def knowledge_summary(plant_id):
        ...
        p=_p()
        with store._connect() as conn:
            cur=conn.cursor(); cur.execute(f"SELECT record_type,COUNT(*) FROM anviqo_plant_knowledge WHERE plant_id={p} AND organization_id={p} GROUP BY record_type ORDER BY COUNT(*) DESC",(plant_id,)); groups=[{"record_type":r[0],"count":r[1]} for r in cur.fetchall()]; cur.execute(f"SELECT COUNT(*) FROM anviqo_plant_knowledge WHERE plant_id={p} AND organization_id={p}",(plant_id,)); total=cur.fetchone()[0]
