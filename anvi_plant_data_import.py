...
Long-running work is queued; the ingestion worker performs the import.
"""
...
VERSION = "ANVIQO-DIRECT-PLANT-DATA-V1.4.8"
BATCH_SIZE = 2000
...
def _insert_batch(plant_id, org_id, document_id, digest, records):
    ...
    for raw in records:
        try:
            ...
            meta.update(import_version=VERSION, document_sha256=digest)
...
def import_plant_data(plant_id, actor):
    ...
    for document_id, filename, digest in docs:
        try:
            with store._connect() as conn:
                cur = conn.cursor()
                cur.execute("SELECT content FROM anviqo_plant_documents WHERE document_id=" + p + " AND plant_id=" + p + " AND organization_id=" + p, (document_id, plant_id, actor["organization_id"]))
                row = cur.fetchone()
            if not row:
            ...
            for rec in _records(filename, raw):
                ...
                if len(batch) >= BATCH_SIZE:
                    a, e = _insert_batch(plant_id, actor["organization_id"], document_id, digest, batch)
                    total += a; errors.extend({"filename": filename, "error": x} for x in e); batch = []
            if batch:
                a, e = _insert_batch(plant_id, actor["organization_id"], document_id, digest, batch)
                total += a; errors.extend({"filename": filename, "error": x} for x in e)
            documents += 1
        except Exception as exc:
...
def enqueue_import(plant_id, actor):
    ...
    with store._connect() as conn:
        ...
        if existing:
            return {"job_id": existing[0], "job_status": existing[1], "existing": True, "mode": "DIRECT_ASYNC", "import_version": VERSION}
...
def get_import_status(plant_id, actor):
    ...
    with store._connect() as conn:
        cur = conn.cursor()
        cur.execute("SELECT job_id,status,message,result_json,created_at,started_at,finished_at,updated_at FROM " + JOB_TABLE + " WHERE plant_id=" + p + " AND organization_id=" + p + " ORDER BY created_at DESC LIMIT 1", (plant_id, actor["organization_id"]))
        r = cur.fetchone()
    if not r:
        return {"status": "NOT_STARTED", "job_status": "NOT_STARTED", "plant_id": plant_id, "mode": "DIRECT_ASYNC", "import_version": VERSION}
...
def register(app):
    ...
    def import_route(plant_id):
        ...
        try:
            ...
            return jsonify({"status": "ERROR", "message": str(exc), "mode": "DIRECT_ASYNC", "safety": dict(SAFETY)}), 400
    ...
    def import_status_route(plant_id):
        ...
        try:
            return jsonify(get_import_status(plant_id, actor)), 200
        except Exception as exc:
    ...
    def knowledge_summary(plant_id):
        ...
        with store._connect() as conn:
            cur = conn.cursor()
            cur.execute("SELECT record_type,COUNT(*) FROM anviqo_plant_knowledge WHERE plant_id=" + p + " AND organization_id=" + p + " GROUP BY record_type ORDER BY COUNT(*) DESC", (plant_id, actor["organization_id"]))
