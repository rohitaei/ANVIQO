...
def _xls_records(raw):
    ...
    try:
        for ws in wb.sheets():
            yield from _matrix_records(ws.name, (ws.row_values(i) for i in range(ws.nrows)))
    finally:
...
def _records(filename, raw):
    ...
    blob = bytes(raw)
    is_xls = blob.startswith(b"\xD0\xCF\x11\xE0\xA1\xB1\x1A\xE1")
    is_xlsx = blob.startswith(b"PK\x03\x04")
    if ext == "xls" or (not ext and is_xls) or (is_xls and ext != "xlsx"):
        yield from _xls_records(blob); return
    if ext == "xlsx" or is_xlsx:
        yield from _xlsx_records(blob); return
    if ext == "csv":
        text = blob.decode("utf-8-sig", errors="replace")
        for i, row in enumerate(csv.DictReader(io.StringIO(text))):
            row = dict(row)
            row["external_id"] = str(row.get("external_id") or row.get("id") or row.get("tag") or row.get("asset_id") or hashlib.sha256((filename + str(i) + json.dumps(row, sort_keys=True, default=str)).encode()).hexdigest()[:20])
            row["source"] = filename; row["record_type"] = str(row.get("record_type") or row.get("entity_type") or row.get("type") or "ASSET")
            yield row
        return
    if ext == "json":
    ...
    if text.strip():
        for i in range(0, len(text), 6000):
            yield {"external_id": hashlib.sha256((filename + str(i)).encode()).hexdigest()[:20], "name": filename, "record_type": "DOCUMENT", "source": filename, "metadata": {"content": text[i:i + 6000]}}
...
def _job_schema():
    with store._connect() as conn:
        ...
        if store._is_sqlite():
            ...
            sql = "CREATE TABLE IF NOT EXISTS " + JOB_TABLE + " (job_id TEXT PRIMARY KEY,organization_id TEXT NOT NULL,plant_id TEXT NOT NULL,actor_json JSONB NOT NULL,status TEXT NOT NULL,message TEXT NOT NULL,result_json JSONB NOT NULL DEFAULT '{}'::jsonb,created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),started_at TIMESTAMPTZ,finished_at TIMESTAMPTZ,updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW())"
        cur.execute(sql)
        cur.execute("CREATE INDEX IF NOT EXISTS idx_" + JOB_TABLE + "_plant ON " + JOB_TABLE + "(plant_id,organization_id,created_at)")
...
def _insert_batch(plant_id, org_id, document_id, digest, records):
    ...
    for raw in records:
        try:
            n = normalize_record(raw, str(raw.get("record_type") or "ASSET"))
            item = asdict(n) if is_dataclass(n) else dict(n)
            meta = dict(item.get("metadata") or {}); meta.update(import_version=VERSION, document_sha256=digest)
            content = str(meta.pop("content", "") or "")
            external_id = str(item.get("external_id", "")); source = str(item.get("source", ""))
            kid = "know_" + hashlib.sha256((plant_id + document_id + external_id + source).encode()).hexdigest()[:24]
    ...
    with store._connect() as conn:
        ...
        if store._is_sqlite():
            sql = "INSERT OR REPLACE INTO anviqo_plant_knowledge(knowledge_id,organization_id,plant_id,document_id,record_type,external_id,name,area,service,asset_type,tag,parent_id,source,metadata,content,created_at) VALUES(" + ",".join(ph) + "," + p + ")"
            cur.executemany(sql, [r + (_now(),) for r in rows])
        else:
            sql = "INSERT INTO anviqo_plant_knowledge(knowledge_id,organization_id,plant_id,document_id,record_type,external_id,name,area,service,asset_type,tag,parent_id,source,metadata,content) VALUES(" + ",".join(ph) + ") ON CONFLICT(plant_id,external_id,source) DO UPDATE SET organization_id=EXCLUDED.organization_id,document_id=EXCLUDED.document_id,name=EXCLUDED.name,area=EXCLUDED.area,service=EXCLUDED.service,asset_type=EXCLUDED.asset_type,tag=EXCLUDED.tag,parent_id=EXCLUDED.parent_id,metadata=EXCLUDED.metadata,content=EXCLUDED.content"
...
def import_plant_data(plant_id, actor):
    ...
    with store._connect() as conn:
        cur = conn.cursor()
        cur.execute("SELECT 1 FROM anviqo_plants WHERE plant_id=" + p + " AND organization_id=" + p + " AND status='ACTIVE'", (plant_id, actor["organization_id"]))
        if not cur.fetchone(): raise PermissionError("Plant is not active in this organization")
        cur.execute("SELECT document_id,filename,sha256 FROM anviqo_plant_documents WHERE plant_id=" + p + " AND organization_id=" + p + " ORDER BY created_at", (plant_id, actor["organization_id"]))
        docs = cur.fetchall()
    total = 0; documents = 0; errors = []
    for document_id, filename, digest in docs:
        try:
            with store._connect() as conn:
                cur = conn.cursor(); cur.execute("SELECT content FROM anviqo_plant_documents WHERE document_id=" + p + " AND plant_id=" + p + " AND organization_id=" + p, (document_id, plant_id, actor["organization_id"]))
                row = cur.fetchone()
    ...
    with store._connect() as conn:
        conn.cursor().execute("UPDATE anviqo_plant_onboarding SET status=" + p + ",updated_at=" + p + " WHERE plant_id=" + p + " AND organization_id=" + p, (status, _now(), plant_id, actor["organization_id"]))
...
def enqueue_import(plant_id, actor):
    ...
    with store._connect() as conn:
        cur = conn.cursor()
        cur.execute("SELECT 1 FROM anviqo_plants WHERE plant_id=" + p + " AND organization_id=" + p + " AND status='ACTIVE' LIMIT 1", (plant_id, actor["organization_id"]))
        if not cur.fetchone(): raise PermissionError("Plant is not active in this organization")
        cur.execute("SELECT job_id,status FROM " + JOB_TABLE + " WHERE plant_id=" + p + " AND organization_id=" + p + " AND status IN('QUEUED','RUNNING') ORDER BY created_at DESC LIMIT 1", (plant_id, actor["organization_id"]))
...
def _claim_job():
    ...
    with store._connect() as conn:
        ...
        if store._is_sqlite():
            ...
            cur.execute("SELECT job_id,organization_id,plant_id,actor_json FROM " + JOB_TABLE + " WHERE status='QUEUED' ORDER BY created_at FOR UPDATE SKIP LOCKED LIMIT 1")
            row = cur.fetchone()
            if not row: return None
...
def _finish_job(job_id, status, result, message):
    p = _p()
    with store._connect() as conn:
        conn.cursor().execute("UPDATE " + JOB_TABLE + " SET status=" + p + ",message=" + p + ",result_json=" + (p + "::jsonb" if not store._is_sqlite() else p) + ",finished_at=" + ("NOW()" if not store._is_sqlite() else p) + ",updated_at=" + ("NOW()" if not store._is_sqlite() else p) + " WHERE job_id=" + p, (status, message, json.dumps(result, default=str), *([_now(), _now()] if store._is_sqlite() else []), job_id))
...
def get_import_status(plant_id, actor):
    _job_schema(); p = _p()
    with store._connect() as conn:
        cur = conn.cursor(); cur.execute("SELECT job_id,status,message,result_json,created_at,started_at,finished_at,updated_at FROM " + JOB_TABLE + " WHERE plant_id=" + p + " AND organization_id=" + p + " ORDER BY created_at DESC LIMIT 1", (plant_id, actor["organization_id"]))
        r = cur.fetchone()
...
def register(app):
    ...
    def knowledge_summary(plant_id):
        ...
        p = _p()
        with store._connect() as conn:
            cur = conn.cursor(); cur.execute("SELECT record_type,COUNT(*) FROM anviqo_plant_knowledge WHERE plant_id=" + p + " AND organization_id=" + p + " GROUP BY record_type ORDER BY COUNT(*) DESC", (plant_id, actor["organization_id"]))
            groups = [{"record_type": r[0], "count": r[1]} for r in cur.fetchall()]
            cur.execute("SELECT COUNT(*) FROM anviqo_plant_knowledge WHERE plant_id=" + p + " AND organization_id=" + p, (plant_id, actor["organization_id"]))
